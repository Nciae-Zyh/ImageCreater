...
export async function classifyIntentAI(
  ...
  try {
    ...
    logger.info(`[Intent] 请求参数: model=${model}, history=${conversationHistory?.length || 0}, mimo=${useMimoParams}`)
    const res = await client.chat.completions.create(reqBody)

    const content = res.choices[0]?.message?.content || '{}'
    logger.info(`[Intent] 原始输出: finish_reason=${res.choices?.[0]?.finish_reason}, content=${content.slice(0, 220)}`)
